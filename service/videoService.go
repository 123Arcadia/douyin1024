...
// 有videoids获取一组视频
func (s VideoService) GetVideoByVideoIds(videoIds []uint) (videoList []models.Video, err error) {
...
func (videoService *VideoService) SaveVideoFile(c *gin.Context, file *multipart.FileHeader, title string, userId uint) error {
	...
	fileDst := utils.GetVideoNewName(userId, userVideoCount+1, file.Filename)
